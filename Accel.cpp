...
bool Accel::collided() {
	int threshold =40;
	//if it detects greater than 0 ms/s squared in any direction, it will return true
	int xDisturbed = abs(this->getX());
	//if x acceleration is greater thna threhsold, return true
	if (xDisturbed > threshold) {
		return true;
	}else {
		return false;
	}
